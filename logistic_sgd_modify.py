...
def sgd_optimization_mnist(learning_rate=0.13, n_epochs=1000,
    ...
    classifier = LogisticRegression(input=x, n_in=1841, n_out=2)
    ...
    while avg_error>0.1:
        ...
        for i in xrange(100):
            print 'round%d'%j
            print 'going to process first batch %d'%i
            minibatch_avg_cost = train_model(i)
            error_sum=error_sum+minibatch_avg_cost
            avg_error=error_sum/(i+1)
            
            print 'error: %f'%minibatch_avg_cost
            print 'done for %d'%(i+1)
            print 'avg_error%f'%avg_error
        #save.append(avg_error)
    end_time = time.clock()
    '''print(
        (
    ...
    weightdata=classifier.W.get_value()
    for i in range(len(weightdata)):
        weight.write(str(weightdata[i]))
    #weight.write(classifier.W.get_value())
    b=open('b.txt','w')
    bvector=classifier.b.get_value()
    for i in range(len(bvector)):
        b.write(str(bvector[i]))
    #b.write(classifier.b.get_value())
    #f=open('st1.txt')
    #line=f.readline()
    #test=T.nnet.softmax(T.dot(line,classifier.W) + (classifier.b))
    #print test
