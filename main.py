...
import ctypes
from multiprocessing import Pipe,Process,Lock,Queue,Manager
...
class gesturedetect(threading.Thread):#手指检测器
    def __init__(self,pipe_send,callback,l):
        threading.Thread.__init__(self)
        self.pipe=pipe_send
                        ...
                        while self.i<self.picsize[0]:
                            if (self.k+self.j<self.picsize[1]/2) and (self.k+self.i<self.picsize[0]):
                                ...
                                if self.pred>=0.5:
                                    ...
                                    self.callback(self.localgesturepool)
                                    break
                            self.i+=20
                        self.j+=10
                    self.k+=20
...
def function_gesturedetect(pipe_send,callback,l):
    g=gesturedetect(pipe_send,callback,l)
    g.start()
...
def callback_fill_gesturepool(localpool):
    global gesturepool
    gesturepool=localpool
...
def mainloop():
    ...
    gparent,gchild=Pipe(True)
    fparent,fchild=Pipe(True)
    g2mparent,g2mchild=Pipe(True)
    f2mparent,f2mchild=Pipe(True)
    #f=fingerdetect()
    #g=gesturedetect()
    #f.setDaemon(True)
    #g.setDaemon(True)
    #f.start()
    #g.start()
    #g_mid=middle(g2mparent)
    #g_mid.start()
    g=Process(target=function_gesturedetect,args=(gchild,callback_fill_gesturepool,l))    
    g.start()
    global started
    while True:
        a,img=capture.read()
