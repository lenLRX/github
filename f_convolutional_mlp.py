...
def load_data(dataset):
    ...
    f.close()
    f2.close()'''
    finger_pos_indexfile='C:\\deeplearning\\pos_finger\\index.txt'
    finger_neg_indexfile='C:\\deeplearning\\neg_finger\\index.txt'
    ...
    finger_pos_rawbuffer=getbuffer(finger_pos_indexfile,'finger_pos.pickle',178,1)
    finger_neg_rawbuffer=getbuffer(finger_neg_indexfile,'finger_neg.pickle',4151,0)
    print '...file loaded'
    train_set=[[],[],[]]    
    for i in range(len(finger_pos_rawbuffer[0])):
        train_set[0].append(finger_pos_rawbuffer[0][i])
        train_set[1].append(finger_pos_rawbuffer[1][i])
        train_set[2].append(0)
    for i in range(len(finger_neg_rawbuffer[0])):
        train_set[0].append(finger_neg_rawbuffer[0][i])
        train_set[1].append(finger_neg_rawbuffer[1][i])
        train_set[2].append(0)
...
def evaluate_lenet5(learning_rate=0.0005, n_epochs=200,
    ...
    cv2.namedWindow('running_CNNf',0)
    IMG=numpy.zeros((480,640))
    cv2.imshow('running_CNNf',IMG)    
    ...
    with open('f_W_layer0.pickle', 'wb') as f:
        pickle.dump(layer0.W.get_value(),f)
    with open('f_B_layer0.pickle', 'wb') as f:
        pickle.dump(layer0.b.get_value(),f)
        
    with open('f_W_layer1.pickle', 'wb') as f:
        pickle.dump(layer1.W.get_value(),f)
    with open('f_B_layer1.pickle', 'wb') as f:
        pickle.dump(layer1.b.get_value(),f)

    with open('f_W_layer2.pickle', 'wb') as f:
        pickle.dump(layer2.W.get_value(),f)
    with open('f_B_layer2.pickle', 'wb') as f:
        pickle.dump(layer2.b.get_value(),f) 
        
    with open('f_W_layer3.pickle', 'wb') as f:
        pickle.dump(layer3.W.get_value(),f)
    with open('f_B_layer3.pickle', 'wb') as f:
